...
import QRCode from "react-qr-code";import { Download } from "lucide-react";
...
const cvTextData = Name:-Agni patel
...
Cert/Skills: IIT BHU (ML & DL).trim();

export default function Cv() {const handleDownloadContact = () => {const blob = new Blob([vcfData], { type: "text/vcard" });const url = URL.createObjectURL(blob);const link = document.createElement("a");link.href = url;link.download = "AGNI_PATEL_Contact.vcf";document.body.appendChild(link);link.click();document.body.removeChild(link);URL.revokeObjectURL(url);};
